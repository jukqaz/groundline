...
- 주간 점검을 사용량 보고에서 사용 패턴·관련 Codex 최신 정보·이전 개선 효과를 연결하는 최적화 루틴으로 확장했습니다. 단일 감사 결과를 재사용하고 GroundLine 제품 개선과 개인 환경 변경의 권한·검증 범위를 구분합니다.
- 개인 지침 실험에 증거 재사용, 필요 시점의 컨텍스트 로딩, 독립 읽기의 제한된 병렬화를 추가했습니다. 실패한 작업의 자원도 포함한 검증 완료 결과당 토큰·시간을 목표로 삼으며 품질 보호와 복구 조건을 유지합니다.
- 개인 실험은 직접 관측한 적용 기회와 지침 로딩·행동 증거의 비공개 해시를 요구합니다. 네이티브 활성화는 별도 검증하며, 누락된 사용량을 0이나 절감량으로 채우지 않습니다.
- 효율 시뮬레이션이 주간 감사 및 결합 주간 리뷰의 관측된 root 사용량을 지원합니다. 고정 가정의 시나리오와 실측 효과를 구분하고, 표본 수를 통계적 신뢰도로 표현하지 않습니다.
- 토큰 세부 항목의 원본 관측 여부를 누계 차이와 합산까지 보존합니다. 시뮬레이션은 출처와 항목별 관측이 확인된 단일 감사만 허용해 누락값과 중복 입력의 허위 절감을 방지합니다.
- 검색·출력 인자에 인용된 테스트 명령을 실제 검증 실행으로 오인하지 않도록 명령 위치 판정을 강화했습니다. 지원하지 않는 동적 실행은 미분류 커버리지로 남기며 과거 서버 이벤트를 재작성하지 않습니다.

BREAKING CHANGE: 개인 outcome·trial 계약은 schema 2입니다. 이전 형식은 자동 변환·삭제하지 않고 명시적으로 거절합니다. 모델 증거와 Insights wire 계약은 유지합니다.

기존 schema 1 실험이 활성화된 호스트는 업그레이드 전에 해당 계약을 지원하는 기존 CLI로 복구하고 실제 지침 해제를 확인해야 합니다. 이전 기록은 보존하며 새 개인 실험은 별도 승인된 비공개 상태 디렉터리에서 시작합니다.

## 0.27.0

- Core·Insights 설치를 하나의 공식 Bash·PowerShell 흐름으로 통합했습니다. 기존 모델·추론 강도·서비스 티어를 보존하며, Astra 설정은 명시적으로 선택할 때만 적용합니다.
- Insights에 재실행 가능한 초기 설정과 연결 검증을 추가했습니다. 기존 연결·수집 동의·식별자·커서를 보존하고, 연결 충돌은 명확히 거절합니다. 수집 동의가 없으면 전송하지 않으며 새 Codex 홈에서 미완성 수집 구간을 만들지 않습니다.
- 알려진 이전 Core 훅 승인 기록만 백업 후 정리합니다. 지원하지 않는 설정은 자동 삭제·변환하지 않고 안내하며, 설치·이전 설정 정리 문서를 통합했습니다.
- 실제 Codex를 이용한 설치·재설치·사용자 설정 보존을 macOS·Linux·Windows의 ARM64·x64 6개 환경에서 검증합니다. Windows 테스트 파일 소유권과 Git Bash 압축 경로 처리를 수정했습니다.

BREAKING CHANGE: `setup`의 기본 동작은 기존 설정 보존입니다. Astra 프리셋은 `--preset astra`로 선택하며, Bash 설치 스크립트의 Codex 실행 경로는 위치 인자 대신 `--codex PATH`로 지정합니다.

## 0.25.9

- Windows의 Git 줄바꿈 변환으로 정상 체크섬 파일이 CRLF가 되었을 때 Core·Insights 설치 검증이 실패하던 문제를 수정했습니다. LF·CRLF 한 줄을 허용하고 실행 파일의 해시·크기·파일명 및 변조 거부 검증은 유지합니다.
- 배포 텍스트는 `.gitattributes`로 LF를 유지하고 네이티브 바이너리는 변환 대상에서 제외합니다. 실제 설치 흐름과 Windows ARM64·x64의 두 플러그인 검증에 회귀 테스트를 추가했습니다.

## 0.25.6

- Astra 지침에서 불필요한 재점검을 줄이고 필요한 검증을 끝까지 수행하도록 명확히 했습니다. 모델·추론·서비스 티어의 사용자 선택은 유지합니다.
- 주간 감사와 Insights 수집의 검증 결과를 네이티브 종료 코드·상태로 판정합니다. 성공 테스트 이름을 실패로 오인하거나 실행 중·미확인 결과를 성공으로 처리하지 않습니다.
- 추천 명령의 `--audit -`에 최대 2 MiB 표준입력을 지원합니다. 주간 감사는 한 번 실행해 결과를 메모리로 전달하며, 활성 설치는 네이티브 목록과 정확한 manifest로 찾습니다.
- 완료 턴과 작업 수, 사용량 커버리지, 알려진 범위 제외와 수집 문제, Guardian 측정 불가 항목을 구분합니다. 이전 서버 이벤트를 삭제·재작성하지 않으며 과거 휴리스틱 지표와 새 지표의 직접 비교를 금지합니다.

## 0.25.5

- 실제 CLI 시작 훅이 만든 정상 기록을 사용량 미측정만으로 격리하던 분류를 수정했습니다. 작업 시작·완료 지표와 수신 관측을 정상 보관 기간 동안 유지하고, 미측정 사용량은 리포트에서 명확히 표시합니다.
- 사용량 출처와 관측 건수가 모순되는 직접 저장을 격리합니다. 불완전한 읽기 결과의 7일 TTL, 원본 JSON·집계 열 일치, 토큰 범위와 기간 검증은 유지합니다.
- 정상 미측정 기록의 보존과 불완전 기록의 만료를 독립 ClickHouse 회귀 테스트로 검증합니다. API 갱신은 기존 원본·이벤트 ID·카운터를 다시 쓰지 않고 뷰와 보관 분류를 바로잡습니다.

## 0.25.4

- 도구 결과 하나에 여러 오류 신호가 있거나 이전 구간의 호출 결과가 뒤늦게 도착해도 정상 집계를 거절하지 않도록 수정했습니다. 신호별 허용 키·정수 범위, 사용량·기간·해시·저장 열 검증은 유지합니다.
- 읽기 완료된 구간이 `audit_failed`로 중단되던 경로를 회귀 검증합니다. API를 먼저 갱신한 뒤 멈춘 구간을 공식 수동 재시도로 복구하며 원본과 수집 커서를 초기화하지 않습니다.
- ingest contract revision 6을 요구해 수정 전 API로 새 이벤트를 전송하는 것을 막습니다. 기존 이벤트 형식과 저장 데이터는 유지합니다.
