...
mod tests {
    ...
    #[tokio::test]
    async fn rejected_report_contract_is_distinct_from_storage_failure_and_private() {
        let response = validated_report_response(json!({"private":"PRIVATE_SENTINEL"}))
            .unwrap_err()
            .into_response();
        assert_eq!(response.status(), StatusCode::UNPROCESSABLE_ENTITY);
        assert_eq!(
            response_json(response).await,
            json!({"status":"FAIL","reason_code":"report_contract_rejected"})
        );
        assert_eq!(
            ApiError::storage().into_response().status(),
            StatusCode::SERVICE_UNAVAILABLE
        );
    }
    ...
    async fn clickhouse_schema_report_and_grafana_queries_are_executable() {
        ...
        let fleet_fixture = REPORT_FLEET_QUERY
            .replace("groundline.release_policy FINAL", "(SELECT 'stable' AS policy_key, '9.9.9' AS latest_version, now() AS updated_at)")
            .replace("groundline.collectors FINAL", "(SELECT toUUID(concat('00000000-0000-4000-8000-', leftPad(toString(number+1),12,'0'))) AS collector_id, now() AS created_at, toUInt32(2) AS enrollment_schema_version, 'linux' AS os_family, 'codex_cli' AS runtime_family, 'local_headless' AS execution_mode, if(number=1,'0.0.1','9.9.9') AS groundline_version, toUInt8(0) AS revoked FROM numbers(3))")
            .replace("groundline.basic_active", "(SELECT toUUID(concat('00000000-0000-4000-8000-', leftPad(toString(number+1),12,'0'))) AS collector_id, now() AS received_at, now() AS generated_at, now() AS period_end, if(number=1,'0.0.1','9.9.9') AS groundline_version FROM numbers(2)) AS fixture_events");
        let now = Utc::now();
        let fleet = clickhouse
            .json_row(
                &fleet_fixture,
                &[
                    ("start", (now - ChronoDuration::days(7)).to_rfc3339()),
                    ("end", (now + ChronoDuration::minutes(1)).to_rfc3339()),
                ],
            )
            .await
            .unwrap()
            .unwrap();
        for (key, expected) in [
            ("enrolled_installation_count", 3),
            ("observed_installation_count", 2),
            ("reporting_installation_count", 2),
            ("never_reported_installation_count", 1),
            ("pending_initial_report_installation_count", 1),
            ("current_package_claim_installation_count", 2),
            ("current_package_claim_unobserved_installation_count", 1),
            ("current_observed_installation_count", 1),
            ("current_reporting_installation_count", 1),
        ] {
            assert_eq!(count(&fleet, key), expected, "{key}");
        }
        for query in [
