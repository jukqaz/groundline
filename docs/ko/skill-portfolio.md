...
| `audit-agent-history` | agent history에서 반복 패턴과 개선점을 찾을 때 |
...
| Skill | 언제 쓰나 |
| --- | --- |
| `agent-ecosystem-radar` | 외부 agent workflow를 조사, 비교, 추천까지 한 번에 묶을 때 |
| `research-agent-ecosystem` | source-backed 후보를 수집할 때 |
| `compare-agent-workflows` | 후보를 GroundLine scope, safety, setup cost 기준으로 비교할 때 |
| `recommend-groundline-upgrades` | adopt, adapt, watch, reject 결정을 만들 때 |
| `evaluate-agent-capability` | tool, skill, plugin, MCP server, hook, agent를 도입 전에 평가할 때 |
| `evaluate-ai-usage-maturity` | 개인이나 팀의 AI 활용도를 증거 기반으로 평가할 때 |
