...
tree, packaged plugin, provider-native validation, staged dogfood, scenario
gate는 통과하지만, 실제 Codex/Claude Code provider target은 현재 draft와
content fingerprint가 달라 full release gate는 PARTIAL로 보는 것이 맞습니다.
명시적인 `--release-version 0.3.3` preflight도 manifest bump와 changelog 이동
전까지 실패하는 것이 맞습니다.
