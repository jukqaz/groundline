...
Antigravity에 설치되고, 19개 skill이 보이며, validation, lint, unit test,
safety eval, offline doctor/radar, provider smoke, staged dogfood, macOS local
scenario, Linux Docker dry-run/execution gate도 통과합니다. v0.3.3 patch
...
1. P0: Version-aware install doctor
   ...
   provider별 설치된 GroundLine version, source ref, cache 상태, skill count를
   확인합니다. provider auth, session, log, raw home dump는 출력하지 않습니다.
...
decision, compact workflow cookbook, artifact lifecycle을 닫았고 source tree와
packaged plugin의 local release gate는 통과했습니다.
