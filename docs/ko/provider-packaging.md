...
- hooks, rules, MCP server, command, provider-level agent를 추가하기 전에는
  `docs/provider-guardrails.md`를 기준으로 검토합니다.
- private/live tool access가 필요할 때는 `docs/mcp-recipes.md`와
  `references/optional-mcp-profiles.md`를 opt-in 경로로 봅니다.
