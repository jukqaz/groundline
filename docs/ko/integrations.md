...
**모든 설치 프로필은 GroundLine Desktop 없이 동작합니다.** GUI는 서버 연결과
전송 상태를 화면에서 관리하고 싶은 사용자의 [별도 선택 설치](../../apps/desktop/README.ko.md#선택-설치)입니다.
GUI를 종료하거나 제거해도 플러그인·수집 동의·서버 설정은 유지됩니다.
Codex의 훅이 설치된 Insights 실행 파일을 직접 호출하므로 로그인 항목이나
GUI의 백그라운드 실행을 켤 필요가 없습니다. 훅이 발생하지 않는 동안의 주기적
